...
_L2:
        ...
        aeL = j;
        aev = l;
        w(Math.max(0L, Math.min(0x2932e00L, (0x2932e00L + aev) - aec.currentTimeMillis())));
        container = new Container(mContext, aeC.getDataLayer(), aeq, l, j);
        if (aeJ != null)
        {
            break; /* Loop/switch isn't completed */
        }
        aeJ = new n(aeC, DC, container, aeF);
_L5:
        if (!isReady() && aeO.b(container))
        {
            b(aeJ);
        }
...
_L3:
    ...
    public void lm()
    {
        ...
        if (c1 != null)
        {
            Container container = new Container(mContext, aeC.getDataLayer(), aeq, 0L, c1);
            b(new n(aeC, DC, container, new _cls1()));
        } else
        {
...
label0:
            ...
            bh.C((new StringBuilder("setting refresh time to current time: ")).append(o.d(aeP)).toString());
            if (!o.e(aeP))
            {
                o.a(aeP, j);
            }
...
_L1:
            if (0x2932e00L + container.getLastRefreshTime() < o.a(aeP).currentTimeMillis()) goto _L4; else goto _L3
_L3:
            return true;
_L4:
            return false;
