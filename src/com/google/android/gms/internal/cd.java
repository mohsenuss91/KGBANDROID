...
public final class cd
{
    ...
    public static boolean a(Context context, cf cf1, cm cm1)
    {
        ...
        if (!TextUtils.isEmpty(cf1.oa))
        {
            ...
            if (as.length < 2)
            {
                ev.D((new StringBuilder("Could not parse component name from open GMSG: ")).append(cf1.oa).toString());
                return false;
            }
        ...
        try
        {
            ev.C((new StringBuilder("Launching an intent: ")).append(intent).toString());
            context.startActivity(intent);
            cm1.T();
        }
        catch (ActivityNotFoundException activitynotfoundexception)
        {
            ev.D(activitynotfoundexception.getMessage());
            return false;
        }
        return true;
    }
