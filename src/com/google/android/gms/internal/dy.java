...
public final class dy extends dx.a
{
    ...
    private static dv a(Context context, az az1, bj bj1, ee ee1, dt dt1)
    {
        ...
        ez.a a1 = s(s2);
        eu.ss.post(new _cls1(context, dt1, ea1, a1, s1));
        ec ec1 = ea1.bs();
    ...
    public static dv a(Context context, String s1, String s2, String s3, ec ec1)
    {
        ...
        {
            ev.D((new StringBuilder("Error while connecting to ad server: ")).append(ioexception.getMessage()).toString());
            return new dv(2);
        }
        url1 = url;
        i = 0;
        ...
        httpurlconnection.disconnect();
        return dv2;
        ev.D((new StringBuilder("Received error HTTP response code: ")).append(j).toString());
        dv1 = new dv(0);
        httpurlconnection.disconnect();
        return dv1;
        eb1.d(map);
    ...
    private static void a(String s1, Map map, String s2, int i)
    {
        if (ev.p(2))
        {
            ev.C((new StringBuilder("Http Response: {\n  URL:\n    ")).append(s1).append("\n  Headers:").toString());
            if (map != null)
            {
                for (Iterator iterator = map.keySet().iterator(); iterator.hasNext();)
                {
                    String s3 = (String)iterator.next();
                    ev.C((new StringBuilder("    ")).append(s3).append(":").toString());
                    Iterator iterator1 = ((List)map.get(s3)).iterator();
                    while (iterator1.hasNext()) 
                    {
                        String s4 = (String)iterator1.next();
                        ev.C((new StringBuilder("      ")).append(s4).toString());
                    }
                }
            ...
            ev.C((new StringBuilder("  Response Code:\n    ")).append(i).append("\n}").toString());
        }
    }
        ...
        implements Runnable
    {
        ...
        _cls1(Context context, dt dt1, ea ea1, ez.a a1, String s1)
        {
            qr = context;
            qs = dt1;
            qt = ea1;
        ...
        implements ez.a
    {
        ...
        public final void a(ey ey1)
        {
            Object aobj[] = new Object[2];
            aobj[0] = "AFMA_buildAdURL";
            aobj[1] = qw;
            String s1 = String.format("javascript:%s(%s);", aobj);
            ev.C((new StringBuilder("About to execute: ")).append(s1).toString());
            ey1.loadUrl(s1);
        }
